...
$p=$_POST['lpwd'];
echo $p;
$q = mysql_query("SELECT pid FROM $tablep  where pwd='$p'") or die("cannot retreive"); 
...
if($c>=2)
{
$q3=mysql_query("SELECT name from $tablep WHERE pid='$s_pid'AND pid NOT IN(SELECT p_pid FROM $tf WHERE $tf.p_pid=$l_pid)UNION(SELECT f_pid FROM $tf WHERE $tf.f_pid=$l_pid) ");
...
print"<input type='hidden' name='lem' value='$p'/>";
}
